...
class DHT22Value(ComponentValue):
    ...
    """
    def __init__(self, created_timestamp: int = 0, temperature: float = 0.0, humidity: float = 0.0):
        self.temperature = temperature
        self.humidity = humidity
        super().__init__(created_timestamp=created_timestamp)
    
    @staticmethod
    def from_json(json_message: str) -> 'DHT22Value':
        """
        ...
        try:
            ...
            print(json_data)
            dht22 = DHT22Value(temperature=float(json_data["temperature"]),
                               humidity=float(json_data["humidity"]),
                               created_timestamp=int(json_data["createdTimestamp"]))
